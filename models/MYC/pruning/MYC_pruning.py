...
pruned_model.load_state_dict(torch.load("./model"))
...
start = time_ns()
predictions(model,test_dataloader,"./example_result.csv")
end =  time_ns()

print("tempo predizione (ns):",end-start)
...
start = time_ns()
predictions(pruned_model,test_dataloader,"./example_result_pruned.csv")
end =  time_ns()
...
# Carica i DataFrame
model_df = pd.read_csv("./example_result.csv")
pruned_df = pd.read_csv("./example_result_pruned.csv")

# Assicurati che gli indici siano allineati
model_df.reset_index(drop=True, inplace=True)
pruned_df.reset_index(drop=True, inplace=True)

# Trova le righe diverse
different_rows = model_df[model_df.ne(pruned_df).any(axis=1)]

# Conta le righe diverse
number_of_different_rows = len(different_rows)

print(f"Numero di righe diverse: {number_of_different_rows}")
