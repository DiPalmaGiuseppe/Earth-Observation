...
teacher = ResNet152(pretrained=True).to(device) 
teacher.train()

student = ResNet50(pretrained=True).to(device) 
teacher.train()
...
optimizer = torch.optim.AdamW(teacher.parameters(), lr=1e-2, weight_decay=5e-4)
...
## run with torch.cat((x2,x3),1) (tensor(2.0896, device='cuda:0'), 0.283810552854313, 1172054526815)
## run with torch.cat((x2,x2,x3),1)
...
start = time_ns()
predictions(teacher,test_dataloader,"./example_result_teacher.csv")
end = time_ns()
...
start = time_ns()
predictions(student,test_dataloader,"./example_result_student.csv")
end =  time_ns()
